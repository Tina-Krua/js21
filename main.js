...
let wordsList = (str, subStr) => {
  let arr = str.
  replace(/[^a-zа-яё\s]/gi, '').toLowerCase().split(' ').filter((arrItem) => arrItem.indexOf(subStr) > -1);
...
let getLocalDate = (date, isSeconds = false, isISO = false) => {
  ...
  if (!isISO) res = isSeconds ? date.toLocaleString()
...
/* let getWeekDay = (d) => {
let date = new Date(d);
let days = new Intl.DateTimeFormat('ru-RU', options).format(date);
...
let car = new Car(3902, 'Pista Spider 488', 'Ferrari', 2019);
let car2 = new Car(1332, 'CLA-Class C118', 'Mercedes', 2018);
